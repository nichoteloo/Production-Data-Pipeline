## S3-To-Redshift with SubDag (Data Quality Check)
...
from plugins.helpers import SqlQueries, DataQualityChecks
...
copy_events_from_s3_to_redshift = StageToRedshiftOperator(
    ...
    table = "staging_events", # also include truncate table columns
    s3_bucket = "<sample-bucket>",
    s3_key = "log_data/{execution_date.year}/{execution_date.month}/{ds}-events.json",
    arn_iam_role = "<sample-arn>"
    region = "us-west-2",
    json_format = "s3://<sample-bucket>/log_json_path.json" # json pattern
)
...
copy_songs_from_s3_to_redshift = StageToRedshiftOperator(
    ...
    arn_iam_role = "<sample-arn>"
    region = "us-west-2",
    json_format = "auto" # grab directly
)
...
data_quality_subdags = {
    table: run_data_quality_check(table, query) \
