## S3-to-Redshift (Staging), without data quality / subdag
### staging needed to perform fact and dimension table queries
...
copy_events_from_s3_to_redshift = StageToRedshiftOperator(
    ...
    s3_key = "<sample-key>" ## log_data
    arn_iam_role = "<sample-arn>", ## arn:aws:iam::506140549518:role/dwhRole
    region = "us-west-2",
    json_format = "s3://<sample-bucket>/log_json_path.json"
)
...
load_user_dimension = LoadDimensionOperator(
    ...
    columns = """
        ...
        gender,
        level
    """, ## list columns user for tables, align with sqlqueries
...
## data warehouse is time dependent, table for time frame
